...
def create_log(*args):
    log = f"Random Forest: {' '.join(map(str,args))}"
    logging.info(log)
    print(log)
    try:
...
def make_predictions(params):
    try:
        ...
        create_log(f"Done for {len(os.listdir('random_forest_tn'))}")
        return evaluation
    except Exception as e:
        create_log(str(e))
...
if __name__ == "__main__":
    ...
    df = pd.DataFrame(op)
    df.to_csv("random_forest_tn/evaluation_rf.csv", index=False)
    send_to_telegram_document("random_forest_tn/evaluation_rf.csv")
    create_log("Done")
